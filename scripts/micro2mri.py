...
datadir_micro  = "/path/to/microscopy/contours"   # directory containing <name_micro>.npz
name_micro     = "lh_registered_contours"
datadir_mri    = "/path/to/mri/meshes"            # directory containing <name_mri>.ply
...
polylines_raw, z_coords = micro_io.load(plot=True)
...
reg = polymorpheo.register_slices("rigid", propag="jacobi", multi="simultaneous", init="centroid", bidir=bidir, xlim=micro_io.xlim, ylim=micro_io.ylim, verbose=False)
polylines_rig, _ = reg.compute(polylines_raw)

reg = polymorpheo.register_slices("affine", propag="jacobi", multi="simultaneous", init="identity", bidir=bidir, xlim=micro_io.xlim, ylim=micro_io.ylim, verbose=False)
...
reg = polymorpheo.register_slices("deformable", propag="jacobi", multi="simultaneous", fit_fun=fit_fun, regul_fun=regul_fun,
                                  niter=1, icp_niter=50, lr=1e-2, wreg=5e-1, sigma=1e-1, int_steps=16, verbose=False,
                                  xlim=micro_io.xlim, ylim=micro_io.ylim, plot=True, tol=1e-4)
...
fig = plot.plot_obj(pts_mri, simps_mri)
fig = plot.plot_obj(pts_micro_aff, simps_micro, pts_col=(1, 0, 0), face_col=(1, 0.5, 0.5), fig=fig)
fig.show()
...
for lr, wreg, sigma, cpts_ratio in zip(lrs, wregs, sigmas, cpts_ratios):
    ...
    fig = plot.plot_obj(pts_mri, simps_mri)
    fig = plot.plot_obj(pts_micro_defo, simps_micro, pts_col=(1, 0, 0), face_col=(1, 0.5, 0.5), fig=fig)
    fig.show()
