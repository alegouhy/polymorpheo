...
class alap():
    ...
    def _vertex_alap(self, vertex_idx, disp, pts):
        ...
        if self.l == 2: energy_per_neighbor = jnp.sum(diff ** 2, axis=-1)
...
def robust_rho(x_sq, alpha, scale, eps=1e-9):
    ...
    if alpha == 2.:
        ...
        beta = jnp.maximum(eps, jnp.abs(alpha - 2.))
        alpha_safe = jnp.where(alpha == 0, eps, 
                               jnp.sign(alpha) * jnp.maximum(eps, jnp.abs(alpha)))
        return (beta / alpha_safe) * (jnp.power(x_sq_scal / beta + 1., 0.5 * alpha) - 1.)
...
def m_estimator(x_sq, est='l2', sigma=1.0):
    ...
    if est == 'l1':
        ...
        x_abs = jnp.sqrt(x_sq)
        dist = jnp.where(x_abs <= sigma, (sigma**2 / 6) * (1 - (1 - x_sq / sigma**2)**3),
                                         sigma**2 / 6)
    
    else:
        raise ValueError(f"Unknown estimator: {est}. Choose from "
